...
bool StringEncryptionPass::processConstantStringUse(Function *F) {
    ...
    // if(F->getName().contains("__global_variable_initializer")){
    ...
    std::map<GlobalVariable *, Value *> localVarMap;
    // DecryptedGV.clear();
    for (BasicBlock &BB : *F) {
        DecryptedGV.clear();
        localVarMap.clear();
            ...
            if (PHINode *PHI = dyn_cast<PHINode>(&Inst)) {
                ...
                for (User::op_iterator op = Inst.op_begin();
                    ...
                    if (GlobalVariable *GV = dyn_cast<GlobalVariable>(*op)) {
                        ...
                        if (DecryptedGV.count(GV) > 0) {
                            if(F->getName().contains("__global_variable_initializer")){
                                auto iter3 = localVarMap.find(GV);
                                if(iter3!=localVarMap.end())
                                    Inst.replaceUsesOfWith(GV,iter3->second);
                            }else{
                                Inst.replaceUsesOfWith(GV, Entry->DecGV);
                            }
                            ...
                            if(F->getName().contains("__global_variable_initializer"))
                            {
                                ...
                                auto charArrayTy = ArrayType::get(IRB.getInt8Ty(),Entry->Data.size());
                                Value * OutBuf = IRB.CreateAlloca(charArrayTy,nullptr,"local_variable_on_stack");
                                Value *Data = IRB.CreateInBoundsGEP(
                                    EncryptedStringTable->getValueType(),
                                    EncryptedStringTable,
                                    {IRB.getInt32(0), IRB.getInt32(Entry->Offset)});
                                localVarMap[GV]=OutBuf;
                                IRB.CreateCall(Entry->DecFunc, {OutBuf, Data});
                            ...
                            //IRBuilder<> IRB1(&F->getEntryBlock());
                            //IRB1.CreateAlloca(IRB1.getInt32Ty(),nullptr,"int32");
                            //IRB1.
                            // Value *OutBuf = IRB.CreateBitCast(
                            //     Entry->DecGV, IRB.getInt8PtrTy());
                            //create local variable in the front of the function for a constant string ,and then decrype it 
